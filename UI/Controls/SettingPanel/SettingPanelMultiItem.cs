...
// String property
    ...
    {
        if (SettingData == null) SettingData = new Dictionary<string, List<string>>();
    }
    ...
    private void FoldAction(object obj)
    {
        ...
        if (Fold)
        {
            FoldBtn.Icon = IconTypes.ChevronDown;
            ToolTip.SetTip(FoldBtn, "展开");
        }
        else
        {
            FoldBtn.Icon = IconTypes.ChevronUp;
    ...
    {
        var value = (string)pi.GetValue(Data);
        ...
        textBox.LostFocus += (e, c) =>
        {
            ...
            if (attribute.IsName) Title = textBox.Text;
