...
public class DataPageViewModel : DataPageModel
{
        ...
        IAppContextMenuServicer contextMenuServicer, IWebData webData)
    {
        ...
        appContextMenuServicer = contextMenuServicer;
    ...
    private void OnTodetailCommand(object obj)
    {
        ...
        if (data != null)
        {
            ...
            {
                main.Data = data.Data as WebSiteModel;
                main.Uri = nameof(WebSiteDetailPage);
            }
        }
    }

    private async void DataPageVM_PropertyChanged(object sender, PropertyChangedEventArgs e)
    {
        ...
        else if (e.PropertyName == nameof(TabbarSelectedIndex))
        {
            ...
            {
                if (YearDate == DateTime.MinValue) YearDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            }
        }
    ...
    private async Task LoadData(DateTime date, int dataType_ = -1)
    {
        ...
        List<ChartsDataModel> chartData = new();
        if (ShowType.Id == 0)
        {
    ...
    private List<ChartsDataModel> MapToChartsWebData(IEnumerable<WebSiteModel> list)
    {
        var resData = new List<ChartsDataModel>();
        try
        {
