...
public class CategoryAppListPageViewModel : CategoryAppListPageModel
{
    ...
    private Task OnSearch(object obj)
    {
        ...
        else if (keyword == "ps") keyword = "Photoshop";
    ...
    private void OnChoosed(object obj)
    {
        ...
        foreach (var item in AppList)
        {
            ...
            {
                //  处理取选
                var isHas = Data.Where(m => m.ID == item.App.ID).Any();
                if (isHas)
                {
                    app.CategoryID = 0;
                    app.Category = null;
    ...
    {
        Category = mainVM.Data as CategoryModel;
    ...
    private Task LoadApps()
    {
        return Task.Run(() =>
        {
            ...
            foreach (var item in appData.GetAllApps())
            {
                ...
                if (app.IsChoosed || item.CategoryID == 0) appList.Add(app);
            }
        ...
        return Task.Run(() =>
            ...
            .ContinueWith(task =>
            {
                if (task.IsCompletedSuccessfully)
                    Dispatcher.UIThread.Invoke(() => { AppList = task.Result; });
            });
    }
}