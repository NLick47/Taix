...
public class Main : IMain
{
    ...
    ///     忽略进程缓存列表
    /// </summary>
    private readonly List<string> _ignoreProcessCacheList;
    ...
    //  已经更新过的应用列表
    private readonly List<string> _updatedAppList = new();
    ...
    public async Task Run()
    {
        ...
        {
            CreateDirectory();
            using var taiDb = new TaiDbContext();
            var tableExists = taiDb.Database
                .ExecuteSqlRaw(
                    "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='__EFMigrationsHistory'") == 1;
            var _dbFilePath = Path.Combine(FileHelper.GetRootDirectory(), "Data", "data.db");
            var fileExistsOrEmpty = File.Exists(_dbFilePath) && new FileInfo(_dbFilePath).Length == 0;
            if (fileExistsOrEmpty || tableExists) await taiDb.Database.MigrateAsync();
            //  加载app信息
    ...
    public void Start()
    {
        ...
        _appTimer.Start();
        _appObserver.Start();
        if (config.General.IsWebEnabled) _webServer.Start();
    ...
    {
        var dir = Path.Combine(FileHelper.GetRootDirectory(), "Data");
    ...
    private bool IsCheckApp(string processName, string description, string file)
    {
        if (string.IsNullOrEmpty(file) || string.IsNullOrEmpty(processName) ||
            DefaultIgnoreProcess.Contains(processName) || _ignoreProcessCacheList.Contains(processName)) return false;

        //  从名称判断
        if (_configIgnoreProcessList.Contains(processName)) return false;

        //  正则表达式
        foreach (var reg in _configIgnoreProcessRegxList)
        ...
        if (config.Behavior.IsWhiteList && config.Behavior.ProcessWhiteList.Count > 0)
        {
            ...
            if (_configProcessNameWhiteList.Contains(processName))
                ...
                //  进程名称中匹配不到时尝试正则表达式
                foreach (var reg in _configProcessRegexWhiteList)
                    if (RegexHelper.IsMatch(processName, reg) || RegexHelper.IsMatch(file, reg))
                    {
                        isWhite = true;
                        break;
        ...
        var app = _appData.GetApp(processName);
        if (app == null)
        {
            //  记录应用信息

            //  提取icon
            var iconFile = Iconer.ExtractFromFile(file, processName, description);
        ...
        Task.Run(async () =>
        {
            try
            {
                ...
                foreach (var link in links)
                    ...
                    {
                        //  属于关联进程
                        foreach (var linkProcess in link.ProcessList)
    ...
    private async void UpdateAppDuration(AppDurationUpdatedEventArgs e)
    {
        ...
        try
        {
            ...
            var startTime = e.ActiveTime;

            var isCheck = IsCheckApp(app.Process, app.Description, app.ExecutablePath);
            if (isCheck)
            {
                //  更新统计时长
    ...
    private void WebSocketEvent_OnWebLog(NotifyWeb args)
    {
        try
        {
            ...
            Task.Run(async () =>
            {
                ...
                var saveName = UrlHelper.GetName(args.Url) + DateTime.Now.ToString("yyyyMM");
                var path = await FaviconDownloader.DownloadAsync(args.Icon, saveName);
    ...
    private void DispatchCateogry(string processName_, string executablePath_)
    {
        try
        {
            var app = _appData.GetApp(processName_);
            if (app != null)
            {
                var categoryList = _categories.GetCategories()
                    .Where(c => c.IsDirectoryMath && c.DirectoryList.Count > 0).ToList();
