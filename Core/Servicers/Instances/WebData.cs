...
{
    private readonly object _createUrlLocker = new();
    ...
    public async Task AddUrlBrowseTimeAsync(Site site_, int duration_, DateTime? dateTime_ = null)
    {
        ...
        try
        {
            if (string.IsNullOrEmpty(site_.Url)) return;
            ...
            //  当前时段时长
            var nowTimeDuration = duration_;
            //  下一个时段时长
            var nextTimeDuration = 0;

            //  当前时段最大使用时长
            var nowTimeMaxDuration = (60 - dateTime.Minute) * 60;
            ...
            {
                //  获取站点信息
                var domain = UrlHelper.GetDomain(site_.Url);
                ...
                if (url == null) throw new Exception("在创建URL时异常");
                ...
                {
                    //  时长
                    var duration = log.Duration + nowTimeDuration;
                    //  判断是否超出
            ...
            if (nextTimeDuration > 0) await AddUrlBrowseTimeAsync(site_, nextTimeDuration, logTime.AddHours(1));
        }
        catch (Exception ex)
        {
    ...
    {
        if (string.IsNullOrEmpty(iconFile_) || string.IsNullOrEmpty(site_.Url)) return;
        //  判断是否是主域名
        var isIndex = UrlHelper.IsIndexUrl(site_.Url);
        ...
        using (var db = new TaiDbContext())
        {
            ...
            //  更新链接图标
            var url = GetCreateUrl(site_);
            if (url != null) url.IconFile = iconFile_;
    ...
    public async Task<IReadOnlyList<WebSiteModel>> GetDateRangeWebSiteListAsync(DateTime start, DateTime end,
        int take = 0, int skip = -1, bool isTime_ = false)
    {
        if (isTime_)
        {
    ...
    public async Task DeleteWebSiteCategoryAsync(WebSiteCategoryModel data_)
    {
        using (var db = new TaiDbContext())
        {
            ...
            foreach (var website in websitesToUpdate) website.CategoryID = 0;
            await db.SaveChangesAsync();
    ...
    public async Task UpdateWebSitesCategoryAsync(int[] siteIds_, int categoryId_)
    {
        ...
        foreach (var site in sitesToUpdate) site.CategoryID = categoryId_;
        await db.SaveChangesAsync();
    }

    public async Task<IReadOnlyList<InfrastructureDataModel>> GetCategoriesStatisticsAsync(DateTime start_,
        DateTime end_)
    {
        using (var db = new TaiDbContext())
        {
    ...
    public async Task<IReadOnlyList<InfrastructureDataModel>> GetBrowseDataStatisticsAsync(DateTime start_,
        DateTime end_, int siteId_ = 0)
    {
        using var db = new TaiDbContext();
        ...
        if (siteId_ > 0) dbResult = dbResult.Where(m => m.SiteId == siteId_);
        ...
        else
        {
            ...
            if (days <= 31)
                ...
                for (var i = 0; i < 12; i++)
                {
                    ...
                    var endTime = new DateTime(date.Year, date.Month, DateTime.DaysInMonth(date.Year, date.Month), 23,
                        59, 59);
    ...
    public async Task<IReadOnlyList<ColumnDataModel>> GetBrowseDataByCategoryStatisticsAsync(DateTime start_,
        DateTime end_)
    {
        using var db = new TaiDbContext();
        var start = new DateTime(start_.Year, start_.Month, start_.Day, 0, 0, 0);
        ...
        var data = await (from wbl in db.WebBrowserLogs
            ...
            group wbl by new { wbl.LogTime, ws.CategoryID }
            into g
            select new CategoryStatisticModel
            {
        ...
        if (start_ == end_)
        {
            ...
            for (var i = 0; i < 24; i++)
            {
                ...
                foreach (var category in categories)
                {
                    var log = data.Where(m => m.CategoryID == category.CategoryID && m.LogTime == time)
                        .FirstOrDefault();
                    if (log != null)
                    {
        ...
        {
            //  获取日期范围数据
            var days = (int)(end_ - start_).TotalDays + 1;
            if (days <= 31)
            {
                //  按天
                foreach (var category in categories)
                ...
                for (var i = 0; i < days; i++)
                {
                    ...
                    foreach (var category in categories)
                    {
                        var duration = data.Where(m =>
                                m.CategoryID == category.CategoryID && m.LogTime >= startTime && m.LogTime <= endTime)
                            .Sum(m => m.Duration);
    ...
    public Task<int> GetBrowseSitesTotalAsync(DateTime start_, DateTime end_)
    {
        ...
        end_ = new DateTime(end_.Year, end_.Month, end_.Day, 23, 59, 59);
        using var db = new TaiDbContext();
        var result = db.WebBrowserLogs.Where(m => m.LogTime >= start_ && m.LogTime <= end_).GroupBy(m => m.SiteId)
            .CountAsync();
        return result;
    }
    ...
    public Task<int> GetBrowsePagesTotalAsync(DateTime start_, DateTime end_)
    {
        ...
        end_ = new DateTime(end_.Year, end_.Month, end_.Day, 23, 59, 59);
        using var db = new TaiDbContext();
        var result = db.WebBrowserLogs.Where(m => m.LogTime >= start_ && m.LogTime <= end_).GroupBy(m => m.UrlId)
            .CountAsync();
        return result;
    }

    public async Task<IReadOnlyList<WebBrowseLogModel>> GetBrowseLogListAsync(DateTime start_, DateTime end_,
        int siteId_ = 0)
    {
        start_ = new DateTime(start_.Year, start_.Month, start_.Day, 0, 0, 0);
        end_ = new DateTime(end_.Year, end_.Month, end_.Day, 23, 59, 59);
    ...
    public async Task ClearAsync(int siteId_)
    {
        ...
        if (websiteToUpdate != null) websiteToUpdate.Duration = 0;
        await db.SaveChangesAsync();
    }
    ...
    {
        //  获取主域名
        var domain = UrlHelper.GetDomain(url_);
    ...
    private async Task ExportWebsiteCsv(string dir, string baseName, List<WebBrowseLogModel> data,
