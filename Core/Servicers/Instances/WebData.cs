...
namespace Core.Servicers.Instances
{
    public class WebData : IWebData
    {
        ...
        public List<InfrastructureDataModel> GetCategoriesStatistics(DateTime start_, DateTime end_)
        {
            using (var db = _database.GetReaderContext())
            {
                ...
                var data = db.Database.SqlQuery<InfrastructureDataModel>(sql).ToList();
                //  未分类
                ...
                var noCategoryData = db.Database.SqlQuery<InfrastructureDataModel>(sql).ToList();
        ...
        public List<InfrastructureDataModel> GetBrowseDataStatistics(DateTime start_, DateTime end_, int siteId_ = 0)
        {
            using (var db = _database.GetReaderContext())
            {
