...
public static class UrlHelper
{
    ...
    public static string GetDomain(string url_, bool isRemovePH_ = true)
    {
        ...
        if (!string.IsNullOrEmpty(ph)) url_ = url_.Replace(ph, string.Empty);

        var index = url_.IndexOf("/");
        if (index != -1) url_ = url_.Substring(0, index);
    ...
    public static string GetName(string url_)
    {
        ...
        //  尝试从已存储的列表中搜索
        if (_domainNamesDict.ContainsKey(result)) return _domainNamesDict[result];
        ...
        if (arr.Length == 2) return FirstLetterToUpper(arr[0]);

        if (arr.Length >= 3)
            return removeStrArr.Contains(arr[0])
                ? FirstLetterToUpper(arr[1])
                : $"{FirstLetterToUpper(arr[1])} {FirstLetterToUpper(arr[0])}";
